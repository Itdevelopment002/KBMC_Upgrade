...
router.put("/departments/:id", (req, res) => {
  const { name, hod, language_code } = req.body;
  const departmentId = req.params.id;

  if (!name || !hod || !language_code) {
    return res.status(400).json({ success: false, message: "Missing required fields (name, hod, or language_code)." });
  }

  const sql = "UPDATE departments SET name = ?, hod = ?, language_code = ? WHERE id = ?";

  db.query(sql, [name, hod, language_code, departmentId], (err, result) => {
    if (err) {
