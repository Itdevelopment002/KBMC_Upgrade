...
const PreviousPresidents = () => {
  ...
  const handleEditClick = (id) => {
    const data = presidents.find((p) => p.id === id);
    setEditData({
      ...data,
  ...
  const handleEditSubmit = async (e) => {
    ...
    formData.append(
      "start_date",
      editData.start_date ? formatDate(editData.start_date) : ""
    );
    formData.append(
      "end_date",
      editData.end_date ? formatDate(editData.end_date) : ""
    );
    ...
    try {
      await api.put(`/presidents/${selectedId}`, formData, {
