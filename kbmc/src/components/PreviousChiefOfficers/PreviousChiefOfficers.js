...
const PreviousChiefOfficers = () => {
  ...
  const fetchChiefs = async () => {
    try {
      const response = await api.get(`/chief-officers?lang=${i18n.language}`);
  ...
  <div className="content-box">
    <h1>{t("previousChiefOfficers.pageTitle")}</h1>
    <ul className="bread-crumb clearfix">
      <li>
        <Link to="/">{t("previousChiefOfficers.breadcrumb.home")}</Link>
      </li>
      <li>
        <span>{t("previousChiefOfficers.breadcrumb.title")}</span>
      </li>
    </ul>
  </div>
</div>
